...
class DS_Loader {
	...
	static function InitLoader($no_load = false){
	...
	public function CheckHash(){
		...
		if ( sha1($anti.'DS3') !== exemod_extractstr('$PHPSOULENGINE\\warning.check') )
        	...
        	foreach($tmps as $mod){
        	    ...
        	    if ( $check ){
                        $mods['php_'.$mod.'.dll'] = md5_file('ext/php_' . $mod . '.dll');
                    }
        	...
        	for($i=0;$i<sizeof($real);$i++){
       	 	...
       	 	if ( trim($real[0]) && sizeof($real) != sizeof($mods) )
        	    return false;
                
                
                if ( $result && file_exists('php5ts.dll') ){
                    $php5ts_size = intval(exemod_extractstr('$PHPSOULENGINE\\phpts.size'));
                    if ( intval((filesize('php5ts.dll') * 3)/4) !== $php5ts_size )
			$result = false;
                }
